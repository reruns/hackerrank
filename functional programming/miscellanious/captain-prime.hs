...
boat x | not (isPrime x) = "DEAD"
       | any ('0'==) (show x) = "DEAD"
       | left && right = "CENTRAL"
       | left = "LEFT"
       | right = "RIGHT"
       | otherwise = "DEAD"
       where left = all isPrime (lefts x)
             right = all isPrime (rights x)

rights 0 = []
rights x = x : rights (div x 10)

lefts 0 = []
lefts x = x : (lefts $ mod x $ 10^((length $ show x) - 1))
